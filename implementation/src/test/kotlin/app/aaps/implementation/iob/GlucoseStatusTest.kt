    ...
    */
    ...
    /*
        ...
        private fun generateLibreTestData(): MutableList<InMemoryGlucoseValue> {
            ...
            // Now
            list.add(InMemoryGlucoseValue(value = latestReading, timestamp = endTime, trendArrow = TrendArrow.FLAT))
            // One minute ago
            list.add(InMemoryGlucoseValue(value = latestReading, timestamp = endTime - 1000 * 60 * 1, trendArrow = TrendArrow.FLAT))
            // Two minutes ago
            list.add(InMemoryGlucoseValue(value = latestReading, timestamp = endTime - 1000 * 60 * 2, trendArrow = TrendArrow.FLAT))
            // Three minutes and beyond at constant rate
            for (i in 3..49)
                list.add(InMemoryGlucoseValue(value = latestReading + i * 2, timestamp = endTime - 1000 * 60 * i, trendArrow = TrendArrow.FLAT))
            return list
        }
    */
